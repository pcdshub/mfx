...
class SimpleDeterministicVH:
    ...
    >>> vh.go_smart('c1', 'rot', 45.0)  # Will detect and handle stuck movement
    """
    ...
    def go_smart(self, c, axis, target, epsilon=0.001, n_iterations_max=10, 
                stuck_threshold=0.001, overshoot_factor=1.2, wait=True):
        """
        Move a motor to target position with smart overshooting when stuck.
        
        This method tracks movement progress and applies overshooting when it detects
        the motor is getting stuck. It will:
        1. Try normal movement first
        2. If movement stalls (position changes less than stuck_threshold), apply overshoot
        3. Return to original target after overshoot
