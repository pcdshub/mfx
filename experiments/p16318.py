...
from mfx.db import RE, bpp, bps, sequencer
import time

target_x = Newport('MFX:HRA:MMN:29', name='target_x')
target_y = Newport('MFX:HRA:MMN:30', name='target_y')
xy = XYGridStage(target_x, target_y, 1, 10, '/cds/home/opr/mfxopr/experiments/mfxp16318/samples.yml')
...
class User:
    ...
    def init_target_grid(self, m, n, sample_name):
        xy = XYGridStage(target_x, target_y, m, n, '/cds/home/opr/mfxopr/experiments/mfxp16318/samples.yml')
        xy.set_presets()
        xy.map_points()
...
#                    sequencer.trigger()
                    yield from bps.trigger_and_read([sequencer, self.xy.x, self.xy.y])
                    time.sleep(0.1)
    ...
    def set_delay(self, delay):
        ...
        if delay <= (opo_time_zero - min_evr_delay - base_inhibit_delay): #formerly 0.16e6
            ...
            inhibit_ec = 210
            ipulse = 0
        elif delay <= (1.e9/120 + opo_time_zero - min_evr_delay - base_inhibit_delay): #formerly 7.e6:
            logger.debug("Triggering on one event code prior")
            inhibit_ec = 211
            ipulse = 1
        elif delay <= (2.e9/120 + opo_time_zero - min_evr_delay - base_inhibit_delay):# formerly 15.5e6
